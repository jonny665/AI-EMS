  ...
  knowledge: [
    {
      ...
      answer:
        "The PoC knowledge base states that students should track completed credits and remaining required modules before graduation. Please confirm official requirements with the academic office.",
    },
    {
      _id: "kb_leave",
      ...
      answer:
        "Students submit a leave request for a specific course session. After approval by an authorised teacher or administrator, the attendance status is updated to on_leave.",
    },
    {
      _id: "kb_evaluation",
      ...
      answer:
        "Course evaluations are shown to teachers as aggregated feedback only. The PoC does not expose individual student identities behind evaluation comments.",
    },
  ],
};
  ...
  if (name === "get-dashboard-data") {
    ...
    return {
      ...
      data: {
        ...
        attendance:
          session.role === "student"
            ? attendance.filter((item) => item.studentId === session.userId)
            : attendance,
        leaveRequests,
...
function submitLeaveFallback(session, data) {
  ...
  const courseOfferingId = String(
    data.courseOfferingId || data.courseId || "",
  ).trim();
  ...
  const courseName = buildCourseName(
    course,
    data.courseName || courseOfferingId,
  );
  let range = null;
  try {
...
function syncAttendanceFallback(leave, now) {
  ...
  const attendance = fallbackState.attendance.find(
    (item) =>
      item.studentId === leave.student_id &&
      item.courseId === leave.course_offering_id &&
      item.date === leave.leave_date,
  );
  const previousStatus = attendance ? attendance.status : "absent";
  const previousSource = attendance
    ? attendance.source || "system_import"
    : "system_import";
  ...
  const existingLink = fallbackState.leaveRequestSessions.find(
    (item) =>
      item.leave_request_id === leave._id &&
      item.class_session_id === classSessionId,
  );
  const link = {
    leave_request_id: leave._id,
    class_session_id: classSessionId,
...
function restoreAttendanceFallback(leave, now) {
  const links = fallbackState.leaveRequestSessions.filter(
    (item) => item.leave_request_id === leave._id,
  );
  const restored = [];
  ...
  for (const link of links) {
    ...
    if (link.attendance_record_id) {
      attendance =
        fallbackState.attendance.find(
          (item) => item._id === link.attendance_record_id,
        ) || null;
    }
    if (!attendance) {
      attendance =
...
function findCourse(courseOfferingId) {
  return (
    fallbackState.courses.find((item) => item._id === courseOfferingId) || null
  );
}
...
function buildCourseName(course, fallbackName) {
  ...
  return code
    ? `${code} ${course.name || fallbackName}`.trim()
...
function buildEvaluationSummary() {
  return fallbackState.courses.map((course) => {
    const evaluations = fallbackState.evaluations.filter(
      (item) => item.courseId === course._id,
    );
    const average = evaluations.length
      ? Math.round(
          (evaluations.reduce((sum, item) => sum + Number(item.rating), 0) /
            evaluations.length) *
            10,
        ) / 10
      : 0;
...
export async function callAiemsFunction(name, data = {}) {
  try {
    ...
    console.warn(
      `[AI-EMS] Cloud function ${name} failed, using local fallback.`,
      error,
    );
    return fallbackResult(name, data);
  }
